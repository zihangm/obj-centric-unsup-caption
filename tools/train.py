...
from captioning.data.dataloader_sentence_to_multi_img import *
import skimage.io
    ...
    try:
        while True:
            if not opt.debug:
                ...
                # ### debug for nan
                # model.model.generator.proj.weight.retain_grad()
                model.att_embed[0].weight.retain_grad()
                ...
                if not (torch.isnan(loss) or torch.isnan(model.att_embed[0].weight.grad).sum()):
                    optimizer.step()
                else:
                    print('Meet nan loss', torch.isnan(model.att_embed[0].weight.grad).sum())
                    # continue
            ...
            if opt.debug or (iteration % opt.save_checkpoint_every == 0 and not opt.save_every_epoch) or \
                ...
                if opt.debug: # if debug or for eval, break after once eval
                    break
