...
class LossWrapper(torch.nn.Module):
    ...
    def forward(self, fc_feats, att_feats, labels, masks, att_masks, gts, gt_indices,
        ...
        if struc_flag:
            ...
            # used for standard (unsupervised) captioning
            output, matching_loss = self.model(fc_feats, att_feats, labels[..., :-1], att_masks)
            # if using UNK, change the mask here for calculating loss
            masks_for_loss = masks
            #if self.opt.use_UNK:
            #    masks_for_loss = masks * (labels != self.opt.vocab_size)
            #else:
            #    masks_for_loss = masks
            # decoder loss
            decoder_loss = self.crit(output, labels[..., 1:], masks_for_loss[..., 1:])
            loss = decoder_loss #+ matching_loss
            # print('Decoder loss:', decoder_loss.mean().item(), 'Matching loss:', matching_loss.mean().item(),
