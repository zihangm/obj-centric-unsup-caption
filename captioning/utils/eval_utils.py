...
def getCOCO(dataset):
    if 'coco' in dataset:
        ...
        annFile = 'coco-caption/annotations/captions_LN_val2014_norepeat.json' # use LN-COCO as ground truth
    elif 'flickr30k' in dataset or 'f30k' in dataset:
        annFile = 'data/f30k_captions4eval.json'
    else:
        print('Annotation set unspecified: Using COCO_LN as default')
        annFile = 'coco-caption/annotations/captions_LN_val2014_norepeat.json'  # use LN-COCO as ground truth
    return COCO(annFile)
...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    # zihang: process preds into int for coco (because my img id is str, but coco annotation is int)
    if 'coco' in dataset:
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        if labels is not None and verbose_loss:
            ...
            with torch.no_grad():
                loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks)[0], labels[..., 1:], masks[..., 1:]).item() # zihang: [0] for first item ([1] is matching loss)
        ...
        with torch.no_grad():
            ...
            try:
                ...
                print('Evaluation of this batch meet error!!!')
                continue
            seq = seq.data
